...
template.promptToSaveAll = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('\t<p>You have unsaved documents open</p><button class="save close">save all</button><button class="close-anyway close">close anyway</button><button class="cancel close">cancel</button>');
  if (!opt_sb) return output.toString();
};
