...
template.documentTableRow = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('\t<li class="document-row ', (opt_data.doc.active) ? 'active' : '', '" data-id="', soy.$$escapeHtml(opt_data.doc.id), '"><div class="file-name">', soy.$$escapeHtml(opt_data.doc.fileName), '</div><div class="directory">', soy.$$escapeHtml(opt_data.doc.directory), '</div></li>');
  if (!opt_sb) return output.toString();
};
